@php
    ...
    if ($role === 'supervisor' && Auth::user()->supervisor) {
        $supervisorPendingCount = \App\Models\MilestoneSubmission::where('status', 'submitted')
            ...
            ->whereHas('student.user', function ($query) {
                $query->whereIn('id', \App\Models\Project::where('supervisor_id', Auth::user()->supervisor->id)->pluck('student_id'));
            })->count();
    }
    ...
    if ($role === 'student' && Auth::user()->student) {
        $studentProject = \App\Models\Project::where('student_id', Auth::user()->student->id)
            ->where('status', 'active')
            ->first();
    }
@endphp
...
<aside class="w-64 min-h-screen bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col">
    ...
    <nav class="flex-1 px-3 py-4 space-y-1">
        ...
        {{-- Supervisor --}}
        @if($role === 'supervisor')
            <p class="px-3 pt-4 pb-1 text-xs font-semibold uppercase tracking-widest text-gray-400 dark:text-gray-500">Students</p>
            ...
            <p class="px-3 pt-4 pb-1 text-xs font-semibold uppercase tracking-widest text-gray-400 dark:text-gray-500">Account</p>
        ...
        @if($role === 'student')
            ...
            @if($studentProject)
                <a href="{{ route('student.projects.show', $studentProject) }}"
                   class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors
