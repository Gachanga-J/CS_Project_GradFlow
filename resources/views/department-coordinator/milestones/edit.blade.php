    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <h3 class="text-lg font-semibold text-gray-700 mb-6">Edit Milestone / Deliverable</h3>

                    <form method="POST" action="{{ route('department-coordinator.milestones.update', $milestone) }}">
                        @csrf
                        @method('PUT')
                            ...
                            <div>
                                ...
                                @if(!empty($cohortOptions))
                                    <select id="intake_year" name="intake_year"
                                        ...
                                        @foreach(array_keys($cohortOptions) as $year)
                                            <option value="{{ $year }}" {{ old('intake_year', $milestone->intake_year) == $year ? 'selected' : '' }}>
                                                {{ $year }} Cohort
                                            </option>
                                        @endforeach
                                    </select>
                                @else
                                    <input type="number" id="intake_year" name="intake_year"
                                           value="{{ old('intake_year', $milestone->intake_year ?? now()->year) }}"
                            ...
                            <div>
                                ...
                                <select id="year_of_study" name="year_of_study"
                                    ...
                                    @foreach([1,2,3,4,5,6] as $year)
                                        <option value="{{ $year }}" {{ old('year_of_study', $milestone->year_of_study) == $year ? 'selected' : '' }}>
                                            Year {{ $year }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="description" name="description" rows="4"
                                      class="w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-indigo-200">{{ old('description', $milestone->description) }}</textarea>
                        </div>
